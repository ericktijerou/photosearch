...
class SearchPhotoUseCaseTest {
    ...
    private val useCase = SearchPhotoUseCase(repository)

    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `GIVEN SearchPhoto WHEN repository returns Success THEN returns data`() = runTest {
        val expected: Flow<PagingData<Photo>> =
            flow { emit(PagingData.from(emptyList())) }
        coEvery { repository.searchPhoto(any()) } returns expected
        val output = useCase("")
        output.first()
        coVerify { repository.searchPhoto(any()) }
        Assert.assertEquals(1, output.count())
    }

    @OptIn(ExperimentalCoroutinesApi::class)
    @Test(expected = Throwable::class)
    fun `GIVEN SearchPhoto WHEN repository returns Failure THEN throw exception`() = runTest {
        val exception = ApiException()
        coEvery { repository.searchPhoto(any()) } throws exception
        useCase("").first()
    }
}